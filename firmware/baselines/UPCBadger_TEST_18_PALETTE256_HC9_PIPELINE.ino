/*
  ...
  WHY:
    The current 324x360 RGB565 CBV carries 16 bits per pixel.
    ...
    ~1.41 levels/channel on the cropped 324x360 image sequence.
  ...
  IMPORTANT:
    Arduino ESP32 SD.h is retained for this test.
...
// Video intentionally uses the proven 324x360 window:
// X=18..341, Y=0..359.
...
#define CBP_FILE "/ConsoleBadger_360_PALETTE256_HC9.CBP"
...
static bool loadCBPIndex(File &file)
{
  ...
  Serial.print("CBP average frame payload: ");
  Serial.println(
    (double)sum / cbpHeader.frameCount,
    1
  );

  Serial.print("CBP max frame payload: ");
...
void setup()
{
  ...
  Serial.println();
  Serial.println("==========================================");
  Serial.println(" UPCBadger TEST 18");
  Serial.println(" PALETTE256 + HC9 + DIRECT-DMA PIPELINE");
  Serial.println("==========================================");

  Serial.println("TFT: 360x360 / 80 MHz");
  Serial.println("Active video window: 324x360 at X=18");
  Serial.println("Codec: 256-colour RGB565 palette + LZ4 HC9");
  Serial.println("Index block: 16 KiB");
  Serial.println("DMA buffers: 2 x 32 KiB");
  Serial.println("Staging: 96 KiB");
  Serial.println("SD request: 40 MHz");
  Serial.println("SD File buffer: DEFAULT");
